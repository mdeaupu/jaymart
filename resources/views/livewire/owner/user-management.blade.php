...
<div class="py-10 mx-auto sm:px-6 lg:px-8">
    ...
    <x-card>
        <div class="p-6">
            <x-table>
                <x-slot name="header">
                    <th class="px-6 py-3 text-sm font-semibold text-gray-800 dark:text-gray-400">Nama</th>
                    <th class="px-6 py-3 text-sm font-semibold text-gray-800 dark:text-gray-400">Email</th>
                ...
                @foreach($users as $user)
                    <tr wire:key="user-{{ $user->id }}" class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                        ...
                        <td class="px-6 py-4 text-sm capitalize"><x-badge
                                color="indigo">{{ $user->getRoleNames()->first() }}</x-badge>
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-800 dark:text-gray-100">{{ $user->branch->name ?? '-' }}</td>
